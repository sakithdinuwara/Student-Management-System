...
<main class="container mx-auto p-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="<%= request.getContextPath() %>/grades/add" class="bg-blue-600 text-white p-2 rounded-md hover:bg-blue-700">Add New Grade</a>
        </div>
        <table class="w-full border-collapse">
            ...
            <c:forEach var="grade" items="${grades}">
                <tr class="hover:bg-gray-50">
                    <td class="border p-2">${grade.gradeId}</td>
                    <td class="border p-2">${grade.studentId}</td>
                    <td class="border p-2">${grade.courseId}</td>
                    <td class="border p-2">${grade.grade}</td>
                    <td class="border p-2">
                        <a href="<%= request.getContextPath() %>/grades/edit?id=${grade.gradeId}" class="text-blue-600 hover:text-blue-800 mr-2">Edit</a>
                        <a href="<%= request.getContextPath() %>/grades/delete?id=${grade.gradeId}" class="text-red-600 hover:text-red-800" onclick="return confirm('Are you sure you want to delete this grade?')">Delete</a>
                    </td>
                </tr>
            </c:forEach>
            <c:if test="${empty grades}">
                <tr>
                    <td colspan="5" class="text-center p-4 text-gray-500">No grades found.</td>
                </tr>
            </c:if>
            </tbody>
        </table>
